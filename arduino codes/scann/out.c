...
void setup() {
  ...
  Serial.println("RFID Reader ready for timeout...");
}
...
void loop() {
  ...
  if (Serial.available() > 0) {
    ...
    // Handle OUT success
    if (command == "SUCCESS_OUT") {
      blinkGreenLEDAndBeep(1, 100, 1); // Blink green LED once, short beep once
    }

    // Handle error for already OUT
    else if (command == "ERROR_ALREADY_OUT") {
      blinkRedLEDAndBeep(2, 100, 2); // Blink red LED twice, short beep twice
    }

    // Handle no previous IN record error during OUT
    else if (command == "ERROR_NO_IN_RECORD") {
      longBeep(); // Long beep
    }

    // Handle unmatched UID error during OUT
